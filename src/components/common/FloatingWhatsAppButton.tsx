 ...
 * Always visible floating button for easy contact
 */
...
class FloatingWhatsAppButton extends Component<{}, FloatingWhatsAppButtonState> {
  ...
  componentDidMount(): void {
    ...
    this.pulseInterval = setInterval(() => {
      this.setState((prevState) => ({ showPulse: !prevState.showPulse }));
    }, 3000);
  }
  ...
  render(): React.ReactNode {
    const { isVisible, showPulse } = this.state;

    if (!isVisible) return null;

    return (
      <button
        className={`${this.baseClass} ${showPulse ? `${this.baseClass}--pulse` : ""}`}
