...
// ============================================================
// Reusable Sub-Components (DRY Principle)
// ============================================================
...
const InfoCard = memo<InfoCardProps>(({ icon, title, children, className = "" }) => {
  return (
    <div className={`storeLocation__card ${className}`}>
      <div className="storeLocation__cardIcon" aria-hidden="true">
        {icon}
      </div>
      <h3 className="storeLocation__cardTitle">{title}</h3>
      {children}
    </div>
  );
});
...
const ActionButton = memo<ActionButtonProps>(({
  ...
  const baseClass = `storeLocation__actionBtn storeLocation__actionBtn--${variant} ${className}`.trim();
  const ariaLabelAttr = ariaLabel || label;
...
const SocialLinks = memo<SocialLinksProps>(({ instagram, tiktok }) => {
...
LocationIconSVG.displayName = "LocationIconSVG";

const PhoneIconSVG = memo(() => (
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
...
PhoneIconSVG.displayName = "PhoneIconSVG";

const ClockIconSVG = memo(() => (
  <svg
    width="24"
    height="24"
    viewBox="0 0 24 24"
...
ClockIconSVG.displayName = "ClockIconSVG";
...
DirectionsIconSVG.displayName = "DirectionsIconSVG";

const WhatsAppIconSVG = memo(() => (
  <svg
    width="16"
    height="16"
    viewBox="0 0 24 24"
...
WhatsAppIconSVG.displayName = "WhatsAppIconSVG";
...
class StoreLocationSection extends Component<StoreLocationSectionProps, StoreLocationSectionState> {
  ...
  public componentDidMount(): void {
    ...
    try {
      if (typeof window !== "undefined" && window.IntersectionObserver && this.sectionRef.current) {
          ...
          (entries) => {
            entries.forEach((entry) => {
              if (entry.isIntersecting) {
                ...
                if (this.intersectionObserver && entry.target) {
        ...
        // Fallback: show immediately if IntersectionObserver not available
        this.setState({ isVisible: true });
      }
    } catch (err) {
      // Error handling: show section even if IntersectionObserver fails
      console.warn("Failed to initialize IntersectionObserver:", err);
      this.setState({ isVisible: true });
    }
  }
  ...
  public componentWillUnmount(): void {
    ...
    if (this.intersectionObserver) {
      try {
        ...
        console.warn("Error disconnecting IntersectionObserver:", err);
      }
  ...
  private copyToClipboard = async (text: string): Promise<void> => {
    ...
    const setCopied = () => {
      this.setState({ copiedAddress: true, isCopying: false });
      if (this.copyResetTimer) {
        window.clearTimeout(this.copyResetTimer);
      }
    ...
    try {
      ...
      const textarea = document.querySelector("textarea[readonly][aria-hidden]");
  ...
  protected renderStoreInfo(data: StoreData): React.ReactNode {
    ...
    if (!data) {
      console.warn("StoreLocationSection: No data provided");
      return null;
    }
    ...
    // Compute values (will be recalculated only when data changes)
    // Sanitize phone number for tel: link
    const phoneForTel = String(phone ?? "").replace(/\s+/g, "").replace(/[^\d+]/g, "");
    const fullAddress = [address, city].filter(Boolean).join(", ");

    const { isVisible } = this.state;
    const visibilityClass = isVisible ? "storeLocation__info--visible" : "";

    return (
      <div
        className={`storeLocation__info ${visibilityClass}`.trim()}
      ...
      >
        ...
        <p className="storeLocation__kicker">Toko & Kontak</p>
        ...
        <div className="storeLocation__cards">
          ...
          <InfoCard
            ...
            title={name || "Lokasi Toko"}
            className="storeLocation__card--location"
          >
            {address && (
            ...
            {!address && !city && (
              <p className="storeLocation__text storeLocation__text--empty">
              ...
              <>
                <div className="storeLocation__actionsRow">
                  <ActionButton
                    ...
                    icon={<CopyIconSVG />}
                    label={this.state.isCopying ? "Menyalin..." : "Salin alamat"}
                    variant="secondary"
                    ariaLabel="Salin alamat toko"
                    className={this.state.isCopying ? "storeLocation__actionBtn--loading" : ""}
                  />
                  <span className="storeLocation__copyHint" aria-live="polite" aria-atomic="true">
            ...
            {mapDirectionsUrl && (
              <ActionButton
                ...
                target="_blank"
                rel="noopener noreferrer"
                icon={<DirectionsIconSVG />}
                label="Petunjuk Arah"
                variant="primary"
                ariaLabel="Petunjuk arah ke toko"
          ...
          <InfoCard
            icon={<PhoneIconSVG />}
            title="Hubungi Kami"
            className="storeLocation__card--contact"
          >
            {phone && (
          ...
          <InfoCard
            ...
            className="storeLocation__card--hours"
          >
            {hours.weekdays && (
        ...
        {social && (social.instagram || social.tiktok) && (
          <SocialLinks instagram={social.instagram} tiktok={social.tiktok} />
        )}
      </div>
    );
  }
  ...
  public render(): React.ReactNode {
    try {
      ...
      if (!data || (!data.name && !data.address)) {
        console.warn("StoreLocationSection: Invalid or missing store data");
        return null;
      }

      const { isVisible } = this.state;
      const sectionClass = `storeLocation ${isVisible ? "storeLocation--visible" : ""}`.trim();
      ...
      // Error boundary: render fallback UI instead of crashing
      console.error("StoreLocationSection render error:", error);
      return (
        <section className="storeLocation" id="Location">
          <div className="storeLocation__container">
            <div className="storeLocation__error">
              <p>Maaf, terjadi kesalahan saat memuat informasi toko.</p>
            </div>
          </div>
        </section>
      );
