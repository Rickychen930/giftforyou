...
function ensureUploadDirectory() {
    try {
        if (!fs_1.default.existsSync(uploadPath)) {
            ...
            try {
                fs_1.default.accessSync(uploadPath, fs_1.default.constants.W_OK);
            }
            catch (accessErr) {
                // Try to fix permissions if possible
                try {
                    fs_1.default.chmodSync(uploadPath, 0o755);
                }
                catch (chmodErr) {
 ...
 */
async function saveUploadedImage(file, retries = 2) {
    // Validate file input
    if (!file) {
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                await new Promise((resolve, reject) => {
                    fs_1.default.writeFile(filePath, bufferToWrite, { mode: 0o644 }, (err) => {
                        if (err)
                            reject(err);
                        else
                            resolve();
